...
class DistributedShardedDataLoader:
    ...
    def next_batch(self):
        ...
        if self.current_position + (B * T * self.num_processes + 1) > len(self.tokens):
            self.advance()
        return x, y , self.current_position , len(self.tokens)
...
def main(
    ...
    tokenizer_path: str = Root_dir+"/Pertokenizer3.model",
    ...
    for step in range(2000):
        ...
        optimizer.step()
        if step % 20 == 0 :
            print(f"step {step}, loss: {loss.item()}, Position: {position}, totalltoken: {totalltoken}")
            model.eval()
